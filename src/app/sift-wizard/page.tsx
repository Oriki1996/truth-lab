...
export default function SIFTWizard() {
  ...
  if (stepIndex >= SIFT_STEPS.length) return (
    <div className="min-h-screen bg-black text-white p-12 flex flex-col items-center justify-center font-sans" dir="rtl">
      <h1 className="text-4xl font-bold text-yellow-400 mb-4">הפכת לצרכן מידע חסין!</h1>
      <p className="mb-8 text-xl">כל הכבוד, סיימת את תרגול ה-SIFT.</p>
      <Link href="/" className="bg-blue-600 hover:bg-blue-700 p-4 px-10 rounded-full font-bold transition-all">חזרה לדף הבית</Link>
    </div>
  );
        ...
        <div className="bg-gray-700 p-6 rounded-lg mb-4">
          {currentStep.id === "S" ? (
            <div className="text-center">
              <p className="text-2xl mb-8 font-serif">"{currentStep.content}"</p>
              <div className="text-3xl font-mono text-yellow-500">{timeLeft > 0 ? `חכה  שניות...` : "אפשר להמשיך"}</div>
              {timeLeft === 0 && <button onClick={() => setStepIndex(stepIndex + 1)} className="mt-8 bg-blue-600 hover:bg-blue-700 p-3 px-8 rounded-lg font-bold">המשך ל-Investigate</button>}
            </div>
          ) : (
            <div>
              <p className="text-xl mb-6">{currentStep.task}</p>
              {currentStep.options.map((opt, i) => (
                <button key={i} disabled={selectedOption !== null} onClick={() => { setSelectedOption(i); setIsCorrect(i === currentStep.correctAnswer); }} 
                className={lock w-full text-right p-4 mb-3 rounded-lg border transition-all \}>{opt}</button>
              ))}
              {isCorrect !== null && (
                <div className="mt-6 p-4 bg-gray-800 rounded-lg">
                  <p className={	ext-lg font-bold \}>{isCorrect ? "נכון מאוד!" : "לא מדויק."}</p>
                  <p className="text-gray-300 mt-2">{currentStep.feedback}</p>
                  <button onClick={() => { setStepIndex(stepIndex + 1); setSelectedOption(null); setIsCorrect(null); }} className="mt-6 bg-blue-600 hover:bg-blue-700 p-2 px-10 rounded-lg font-bold">לשלב הבא</button>
                </div>
              )}
            </div>
          )}
