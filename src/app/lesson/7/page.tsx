    ...
    <main className="min-h-screen bg-slate-50 p-8 text-right" dir="rtl">
      <div className="max-w-4xl mx-auto bg-white p-8 rounded-3xl shadow-xl border border-slate-100 font-sans">
        ...
        <nav className="flex gap-4 mb-8">
          <button onClick={() => setActiveTab('gan')} className={px-4 py-2 rounded-lg font-bold \}>איך זה עובד? (GAN)</button>
          <button onClick={() => setActiveTab('mcgurk')} className={px-4 py-2 rounded-lg font-bold \}>אפקט מקגורק</button>
        </nav>
        ...
        <a href="/" className="mt-12 inline-block text-blue-600 font-bold">→ חזרה ללוח הבקרה</a>
      </div>
    </main>
  );
}
